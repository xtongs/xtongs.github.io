...
$(function () {
  ...
  $.widget("flowchart.flowchart", {
    ...
    options: {
      ...
      defaultClasses: ['flowchart-operator', 'fixed', 'horizontal', 'ui-draggable', 'hover', 'selected'],
      defaultLinkColor: '#20a0ff',
      defaultSelectedLinkColor: '#555',
      linkWidth: 6,
      grid: 20,
    ...
    getConnectorPosition: function (operatorId, connectorId, subConnector) {
      ...
      var width = (parseInt($connector.css('border-width')) / Math.sqrt(2)) * 2;
      var x = (connectorOffset.left - elementOffset.left) / this.positionRatio + width;
      var y = (connectorOffset.top - elementOffset.top) / this.positionRatio + width;
    ...
    _refreshLinkPositions: function (linkId) {
      ...
      var offsetFromY = offsetFromX;
      var fromX = horizontal ? fromPosition.x + offsetFromX - distanceFromArrow : fromPosition.x;
      var fromY = horizontal ? fromPosition.y : fromPosition.y + offsetFromY - distanceFromArrow;
      var toX = horizontal ? toPosition.x - offsetFromX + distanceFromArrow : toPosition.x;
      var toY = horizontal ? toPosition.y : toPosition.y - offsetFromY + distanceFromArrow;
      var bezierIntensity = Math.min(100, Math.max(Math.abs(fromY - toY), Math.abs(fromX - toX)));

      if (horizontal) {
        // horizontal
        linkData.internal.els.path.setAttribute("d", 'M' + fromX + ',' + fromY + ' C' + (fromX + bezierIntensity) + ',' + fromY + ' ' + (toX - bezierIntensity) + ',' + (toY) + ' ' + toX + ',' + toY);
      } else {
        // vertical
        linkData.internal.els.path.setAttribute("d", 'M' + fromX + ',' + fromY + ' C' + fromX + ',' + (fromY + bezierIntensity) + ' ' + toX + ',' + (toY - bezierIntensity) + ' ' + toX + ',' + toY);
      }
    },
    ...
    _getOperatorFullElement: function (operatorData) {
      ...
      if (this.options.horizontal) {
        $operator.addClass('horizontal')
      }
      if (this.options.fixedWidth) {
        $operator.addClass('fixed')
      }
      ...
      var $operator_subtitle = $('<div class="flowchart-operator-subtitle"></div>');
      $operator_subtitle.html(infos.subtitle);
      if (infos.subtitle) {
        $operator.addClass('subtitle')
      }
      $operator_subtitle.appendTo($operator_title);
    ...
    _connectorClicked: function (operator, connector, subConnector, connectorCategory) {
      if (connectorCategory == 'outputs') {
        ...
        var x = position.x + (this.options.horizontal ? position.width / 2 : 0);
        var y = position.y + (!this.options.horizontal ? position.width / 2 : 0);
    ...
    addClassOperator: function (operatorId, className) {
      ...
      var operatorEl = operator.internal.els.operator;
      operatorEl.addClass(className);
      var classes = operatorEl.attr('class').split(' ').filter((c) => {
        return this.options.defaultClasses.indexOf(c) === -1
      }).join(' ');

      operator.properties.class = classes;
      this._refreshInternalProperties(operator);
    },

    removeClassOperator: function (operatorId, className) {
      var operator = this.data.operators[operatorId]
      var operatorEl = operator.internal.els.operator;
      operatorEl.removeClass(className);
      var classes = operatorEl.attr('class').split(' ').filter((c) => {
        return this.options.defaultClasses.indexOf(c) === -1
      }).join(' ');
